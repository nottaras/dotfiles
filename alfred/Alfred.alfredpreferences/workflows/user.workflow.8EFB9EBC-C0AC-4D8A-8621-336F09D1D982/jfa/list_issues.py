...
def get_result_list(query_str: str) -> list[Result]:
    ...
    try:
        ...
        if "does not exist" in str(e) or "permission" in str(e):
            return []
        else:
            raise

    return [get_result_from_issue(issue) for issue in issues]
...
def main(query_str: str) -> None:
    try:
        ...
        if is_issue_url(query_str):
            ...
            query_str = f"TESP-{query_str}"

        results = get_result_list(query_str)

        if not results:
            issue_url = get_issue_url(query_str)
            results.append(
                {
                    "title": "No Results Found",
                    "subtitle": f"Press ⏎ to open {issue_url} in browser",
                    "arg": issue_url,
                    "valid": True,
                    "icon": {"path": "icon.png"},
                }
